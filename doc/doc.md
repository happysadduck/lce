...
### IV-6-1. 监听友军
类似于发现敌军. 监听友军使用同样高明的方式.
计划管理器除了存储一堆计划以外, 还存储了"正在接收友军信息的"的船. 以船本身为索引, 存储"发送的信息", "发送者", "发送时间"这三个重要字段. 
在每一帧更新地图的时候, 当遍历到某个船的时候, 就检查它是否有友军船只给它发送信息. 如果有, 那就观测该友军飞船. 当观测到的时刻晚于信息发送的时刻, 那就说明信息送到了. 
由于可能有不止一个友军船只发送信息, 因此实际上存储的是一个链表. 需要每次都遍历整个链表中的所有船来验证信息是否送到. 如果送到了, 那就根据信息更新地图或者计划管理器, 然后删除链表上的该节点. 
### IV.6-2. 命令结构
