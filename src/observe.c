    ...
    const Point *s2)
{
    ...
    for (int i = 0; i < 2; i++)
    {
        for (int j = 0; j < 2; j++)
        {
            ...
            {
                if (
                    fmax(bd[2 * i], bd[2 * i + 1]) >= side[2 * i] &&
                    fmin(bd[2 * i], bd[2 * i + 1]) <= side[2 * i + 1])
                    return 1;
            }
        }
    }
    ...
    Discovery *out)
{
        ...
        t--)
    {
        ...
        double px = Bx - Vx * dt_sec - 0.5 * Ax * dt_sec * dt_sec;
        double py = By - Vy * dt_sec - 0.5 * Ay * dt_sec * dt_sec;
        double dx = Ox - px;
        double dy = Oy - py;
    ...
    Discovery *out)
{
    ...
    while (R > max_backtrace_tick)
    {
        ...
        Point pL, vL;
        pL.x = p_curr.x - v_curr.x * dt_sec - 0.5 * a_curr.x * dt_sec * dt_sec;
        pL.y = p_curr.y - v_curr.y * dt_sec - 0.5 * a_curr.y * dt_sec * dt_sec;
