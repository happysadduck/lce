    ...
    double *out)
{
    ...
    if (a_norm2 < eps)
    {
        double Aq = (V.x * V.x + V.y * V.y) - ev * ev;
        double Bq = 2.0 * ((D.x * V.x + D.y * V.y) -
                           r0 * ev);
        double Cq = (D.x * D.x + D.y * D.y) - r0 * r0;
    ...
    else
    {
        ...
        double a2 = 4.0 * ((D.x * A.x + D.y * A.y) +
                           (V.x * V.x + V.y * V.y) -
                           ev * ev);
        double a1 = 8.0 * ((D.x * V.x + D.y * V.y) - r0 * ev);
        double a0 = 5.0 * ((D.x * D.x + D.y * D.y) - r0 * r0);
